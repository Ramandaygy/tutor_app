...
const NumerikChatbot = () => {
    ...
    const fetchHistory = async () => {
      ...
      try {
        ...
        if (data.messages?.length > 0) {
          ...
          const cached = localStorage.getItem("numerik_messages");
  ...
  useEffect(() => {
    const saveHistory = async () => {
      ...
      localStorage.setItem("numerik_messages", JSON.stringify(messages));
  ...
  const handleSoalBaru = async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:5000/chatbot/get_question?theme=numerik&user_id=${userId}`
      );
    ...
    <div className="chatbot-container">
      ...
      <div className={`chatbot-history ${sidebarVisible ? "show" : ""}`}>
        ...
        <div className="chatbot-main">
          <div className="numerik-header">
            <h2>Ruang Belajar - numerik</h2>
